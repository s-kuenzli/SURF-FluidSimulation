...
class GraphViT(nn.Module):
    def __init__(self, state_size, w_size=512, n_attention=4, nb_gn=4, n_heads=4, parameter_size=2):
        super(GraphViT, self).__init__()
        pos_start = -3
        pos_length = 8
        self.encoder = Encoder(nb_gn, state_size, pos_length, parameter_size)
    ...
    def forward(self, mesh_pos, edges, state, node_type, clusters, clusters_mask, parameters, apply_noise=False):
        if apply_noise:
        ...
        for t in range(1, state.shape[1]):
            ...
            V, E = self.encoder(mesh_pos[:, t - 1], edges[:, t - 1], state_hat[-1], node_type[:, t - 1], mesh_posenc, parameters[:, t - 1])
...
class Encoder(nn.Module):
    def __init__(self, nb_gn=4, state_size=3, pos_length=7, parameter_size=2):
        super(Encoder, self).__init__()
        self.encoder_node = MLP(input_size=9 + state_size + parameter_size, output_size=128, n_hidden=1, layer_norm=False)
    ...
    def forward(self, mesh_pos, edges, states, node_type, pos_enc, parameters):
        V = torch.cat([states, node_type, parameters], dim=-1)
