...
class FluentDataset:
    def __init__(self):
        ...
        self.dataloc = []
        path = os.path.join(self.fn)
        
        for example in os.listdir(path):
            folder = os.path.join(path, example)
            if "Splits" in example:
                continue
            if os.path.isdir(folder):
                print(folder)
                self.dataloc.append(folder)
...
def constrained_clustering_numpy(pointcloud, init_clusters, n_cluster, max_cluster_size):
    ...
    for _ in range(1000):
        ...
        #                                                  max_cluster_size)
        clusters, number_of_swaps, cluster_centers = swap(pointcloud, clusters, n_cluster, n_points,
                                                          max_cluster_size)
...
def main():
    ...
    for i in range(len(dataloader)):
        print(f"Processing {dataloader.dataloc[i]} ({args.max_cluster_size}):")
